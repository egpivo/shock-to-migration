...
use crate::flow::inclusive_day_count;
...
mod tests {
    ...
    fn price_return_and_volume_ratio() {
        ...
        let s = account_market_response(series, &window(), &baseline).unwrap();
    ...
    fn missing_prices_stay_none_not_zero() {
        ...
        let s = account_market_response(series, &window(), &[]).unwrap();
    ...
    fn rejects_duplicate_day() {
        ...
        let err = account_market_response(series, &window(), &[]).unwrap_err();
